...
#[command]
async fn visa(ctx: &Context, msg: &Message, args: Args) -> CommandResult {
    // Run the Python script to get the case status and details
    let case_number = args.current().unwrap_or("WAC2390058528").to_string();
    ...
    match output {
        Ok(output_data) if output_data.status.success() => {
            ...
            let form = visa_data["form"].as_str().unwrap_or("Not found");
            let case_id = visa_data["case_id"].as_str().unwrap_or("Not found");
            let case_history_default = vec![];
            let case_history = visa_data["case_history"].as_array().unwrap_or(&case_history_default);

            let header = format!("Latest Case Status: {}", latest_case_status);
            let formatted_details = details
            .replace("<a href=\"", "")
            .replace("\" target=\"_blank\">", "")
            .replace("</a>", "");            let form_type = format!("{}", form);
            let case = format!("{}", case_id);
            let history = case_history
                .iter()
                .map(|entry| format!("Date: {}, Status: {}", entry["date"], entry["status"]))
                .collect::<Vec<String>>()
                .join("\n");
            ...
            .channel_id
            .send_message(&ctx.http, |m| {
                m.content(format!("USCIS Case Status for {}:", case_number))
                    .embed(|e| {
                        e.title(header)
                            .description(formatted_details)
                            .thumbnail("https://egov.uscis.gov/processing-times/img/SocialMedia_SignatureGraphics_800x800.png")
                            .fields(vec![
                                ("Case Number", case, true),
                                ("Form Type", form_type, true),
                                //("This is the second field", "Both fields are inline", true),
                                ])
                            .field("Case History", history, false)
                            .footer(|f| f.text("Generated by PomRust"))
                            .timestamp(timestamp)
